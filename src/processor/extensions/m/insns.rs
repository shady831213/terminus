...
fn bigint_to_reg(b: BigInt, size: usize) -> RegT {
    ...
    let mut value = v[..min(size, v.len())]
        .iter()
        .enumerate()
    ...
    if v.len() < size {
        for p in v.len()..size {
            value.set_bit_range(
                (p << 3) + 7,
                p << 3,
                if b.sign() == Sign::Minus { 0xff } else { 0x0 },
            )
        }
    }
    value
...
impl Execution for MULH {
    fn execute(&self, p: &mut Processor) -> Result<(), Exception> {
        p.state().check_extension('m')?;
        let rs1 = BigInt::from(sext(
            *p.state().xreg(self.rs1(p.state().ir())),
            p.state().config().xlen.len(),
        ) as SRegT);
        let rs2 = BigInt::from(sext(
            *p.state().xreg(self.rs2(p.state().ir())),
            p.state().config().xlen.len(),
        ) as SRegT);
        let rd = self.rd(p.state().ir());
        let value = bigint_to_reg(
            (rs1 * rs2) >> p.state().config().xlen.len(),
            p.state().config().xlen.size(),
...
impl Execution for MULHSU {
    fn execute(&self, p: &mut Processor) -> Result<(), Exception> {
        ...
        let rs2 = BigInt::from(
            *p.state().xreg(self.rs2(p.state().ir())) & p.state().config().xlen.mask(),
        );
        let product = (rs1 * rs2) >> p.state().config().xlen.len();
        let rd = self.rd(p.state().ir());
        let value =
            bigint_to_reg(product, p.state().config().xlen.size()) & p.state().config().xlen.mask();
        let pc = *p.state().pc() + 4;
        p.state_mut().set_xreg(rd, value);
        p.state_mut().set_pc(pc);
        Ok(())
...
impl Execution for MULHU {
    fn execute(&self, p: &mut Processor) -> Result<(), Exception> {
        p.state().check_extension('m')?;
        let rs1 = BigInt::from(
            *p.state().xreg(self.rs1(p.state().ir())) & p.state().config().xlen.mask(),
        );
        let rs2 = BigInt::from(
            *p.state().xreg(self.rs2(p.state().ir())) & p.state().config().xlen.mask(),
        );
...
impl Execution for DIV {
    fn execute(&self, p: &mut Processor) -> Result<(), Exception> {
        p.state().check_extension('m')?;
        let rs1: Wrapping<SRegT> = Wrapping(sext(
            *p.state().xreg(self.rs1(p.state().ir())),
            p.state().config().xlen.len(),
        ) as SRegT);
        let rs2: Wrapping<SRegT> = Wrapping(sext(
            *p.state().xreg(self.rs2(p.state().ir())),
            p.state().config().xlen.len(),
        ) as SRegT);
...
impl Execution for DIVW {
    fn execute(&self, p: &mut Processor) -> Result<(), Exception> {
        p.state().check_extension('m')?;
        p.state().check_xlen(XLen::X64)?;
        let rs1: Wrapping<SRegT> =
            Wrapping(sext(*p.state().xreg(self.rs1(p.state().ir())), 32) as SRegT);
        let rs2: Wrapping<SRegT> =
            Wrapping(sext(*p.state().xreg(self.rs2(p.state().ir())), 32) as SRegT);
        if rs2 == Wrapping(0 as SRegT) {
            let rd = self.rd(p.state().ir());
            let value = (-1 as SRegT) as RegT & p.state().config().xlen.mask();
            p.state_mut().set_xreg(rd, value);

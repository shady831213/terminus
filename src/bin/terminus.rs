...
use terminus::devices::virtio_console::{VirtIOConsole, VirtIOConsoleDevice};
#[cfg(feature = "sdl")]
use terminus::devices::virtio_input::{VirtIOKb, VirtIOKbDevice};
#[cfg(feature = "sdl")]
...
fn main() {
    ...
    let core_num = usize::from_str(matches.value_of("core_num").unwrap_or_default())
        .expect("-p expect a decimal int");
    let memory_size = u64::from_str_radix(
        matches
            .value_of("memory")
            .unwrap_or_default()
    ...
    #[cfg(feature = "sdl")]
    let display_width = u32::from_str(matches.value_of("width").unwrap_or_default())
        .expect("width expect a decimal");
    #[cfg(feature = "sdl")]
    let display_height = u32::from_str(matches.value_of("height").unwrap_or_default())
        .expect("height expect a decimal");
    #[cfg(feature = "sdl")]
    ...
    let step = match usize::from_str(matches.value_of("step").unwrap_or_default())
        .expect("step expect a decimal")
    {
    ...
    let mut trace_file = if matches.is_present("trace") || trace_all {
        Some(
            OpenOptions::new()
                ...
                .write(true)
                .open("terminus.trace")
                .expect("Can not open terminus.trace!"),
        )
    } else {
        None
    };
    ...
    let main_memory = GHEAP
        .alloc(memory_size, 1)
        .expect("main_memory alloc fail!");
    sys.register_memory("main_memory", 0x80000000, &main_memory)
        .unwrap();
    sys.register_device("clint", 0x02000000, 0x000c0000, Clint::new(sys.timer()))
        .unwrap();
    sys.register_device("plic", 0x0c000000, 0x4000000, Plic::new(sys.intc()))
        .unwrap();
    ...
    let (sdl, fb, kb, mouse) = {
        if display_en {
            let sdl = SDL::new(
                ...
                display_height,
                pixel_format,
                || EXIT_CTRL.exit("sdl exit!").unwrap(),
            )
            .expect("sdl open fail!");
            let fb = Rc::new(Fb::new(display_width, display_height, pixel_format));
            let irq_num = sys.intc().num_src();
            let kb = Rc::new(VirtIOKbDevice::new(
                &virtio_mem,
                sys.intc().alloc_src(irq_num),
            ));
            sys.register_virtio("virtio_keyboard", VirtIOKb::new(&kb))
                .unwrap();
            let irq_num = sys.intc().num_src();
            let mouse = Rc::new(VirtIOMouseDevice::new(
                &virtio_mem,
                sys.intc().alloc_src(irq_num),
            ));
            sys.register_virtio("virtio_mouse", VirtIOMouse::new(&mouse))
                .unwrap();
    ...
    let irq_num = sys.intc().num_src();
    let virtio_console_device = Rc::new(VirtIOConsoleDevice::new(
        &virtio_mem,
        sys.intc().alloc_src(irq_num),
    ));
    sys.register_virtio("virtio_console", VirtIOConsole::new(&virtio_console_device))
        .unwrap();
    ...
    if let Some(image_file) = image {
        ...
        let virtio_blk = VirtIOBlk::new(
            ...
            sys.intc().alloc_src(irq_num),
            8,
            Path::new(image_file).to_str().expect("image not found!"),
            VirtIOBlkConfig::new(matches.value_of("image_mode").unwrap_or_default()),
        );
        sys.register_virtio("virtio_blk", virtio_blk).unwrap();
    }
    let virtio_net_device = if let Some(tap) = net {
        let irq_num = sys.intc().num_src();
        let virtio_net_device = Rc::new(VirtIONetDevice::new(
            &virtio_mem,
            sys.intc().alloc_src(irq_num),
            tap,
            0x0100_00000002,
        ...
        sys.register_virtio("virtio_net", VirtIONet::new(&virtio_net_device))
            .unwrap();

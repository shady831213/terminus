...
fn main() {
    ...
    let mut sys = System::new("sys", Path::new("examples/linux/image/br-5-4").to_str().expect("image not found!"), configs, 10000000);
    sys.register_memory("main_memory", 0x80000000, &GHEAP.alloc(0x8000000, 1).expect("main_memory alloc fail!")).unwrap();
